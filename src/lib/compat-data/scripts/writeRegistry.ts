...
const readWebFeature = (id: string): WebFeatureStatus | undefined => {
  ...
  return {
    ...
    // Spread rather than assigned: under `exactOptionalPropertyTypes` an absent date and one explicitly `undefined`
    // are different types, and the upstream status has the first.
    ...(status.baseline_low_date === undefined ? {} : { baseline_low_date: status.baseline_low_date }),
    ...(status.baseline_high_date === undefined ? {} : { baseline_high_date: status.baseline_high_date }),
  };
};
...
// `fromEntries` answers a string-keyed bag; the range it is built from is exactly the key union, which is what the
// annotation says and what lets every lookup downstream skip a fallback nothing could reach.
const ageWindowTargets = Object.fromEntries(
  AGE_WINDOW_YEARS_RANGE.map((years) => {
    return [String(years), targetForWindow(years)];
  }),
) as Readonly<Record<`${AgeWindowYears}`, BrowserTarget>>;
...
export const ageWindowTargets: Readonly<Record<\`\${AgeWindowYears}\`, BrowserTarget>> = ${
